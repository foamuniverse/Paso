...
The resulting consensus mechanism requires no specialized hardware, no energy expenditure beyond normal smartphone operation and human activity, and distributes block production capacity across all mobile participants far more equally than any prior consensus mechanism. The network uses a stateless coin model in which each unspent output carries its own spending conditions as a self-contained program, supporting programmable coins, issued assets, and atomic swaps on a base layer designed to be validatable on commodity smartphones. The system replaces the central authoritative ledgers that conventional finance is built around — the single administered database that is its point of control, failure, and cost — without claiming to eliminate trust.

## 1. Introduction

Conventional financial infrastructure is organized around siloed authoritative databases: a bank's record of its balances, a card network's record of transactions, a clearing house's register of obligations. Each is owned and administered by trusted institutions, and represents a single point of control or failure — a target that can be breached with  compromised credentials, entrusted to administrators who can manipulate or corrupt records inadvertently, and a reconciliation machinery that must exist because every institution keeps their own copies which might diverge. A distributed ledger removes any such choke points. Records are maintained jointly: no single target exists, and reconciliation between competing copies is automatic.

Maintaining such a distributed ledger without competing operators, each individually fallible or beholden to separate instructions, is the work of a consensus mechanism. Existing consensus mechanisms allocate block production rights according to expenditure of some scarce resource: computation in Proof of Work, staked capital in Proof of Stake, allocated storage in Proof of Space. In practice, these resources are economically accumulable and exhibit strong economies of scale, producing power-law distributions in which participants with greater capital capture disproportionate consensus influence. Scarcity alone thus doesn't sidestep concentration, but some scarce recources resist — votes and rations are scarce yet distributed by design rather than by accumulation. What concentrates the resources leveraged for existing consensus mechanisms is the combination of accumulability and capital-driven returns to scale.

Human locomotion time differs structurally from scalable resources. Temporal and practical constraints sharply limit accumulation: Financial independence might permit additional walking hours per day relative to the individual bound by wage labor, but accumulation is limited by finite hours in a day.
