...
Conventional financial infrastructure is organized around central authoritative databases: a bank's record of its balances, a card network's record of transactions, a clearing house's register of obligations. Each is owned and administered by a trusted institution, and each represents a single point of control, failure — a target that can be breached with credentials that can be compromised, administrators who can manipulate records, or corrupt them inadvertently, and a reconciliation machinery that must exist because every institution keeps limited copies. A distributed ledger removes the central database. The record is maintained jointly: no single target, no privileged administrator and automated reconciliation between competing copies.
